...
const isEmpByFamilyMember = ref(null);
const borrowersStore = useBorrowersStore();
const employmentStore = useEmployMentStore();
const route = useRoute();

const setRelationship = (bool) => {
  isEmpByFamilyMember.value = bool;
...
const specialBorrowerEmployerRelationshipIndicatorValue = computed(() => {
  if (currentEmployment.value.EmployedByFamilyMember === null) {
    return null;
  } else {
    const isEmployed = currentEmployment.value.EmployedByFamilyMember;
    isEmpByFamilyMember.value = isEmployed;
    return isEmployed;
  }
});
...
<template>
  <div class="content-container image-content-type">
    <div class="image-container">
      <div class="titleText">{{ mainTitleVerb }} you employed by a family member?</div>
      <PartyToTransaction />
      <v-form class="my-5">
        <AgreeToggle
    ...
    <div class="navigation-container">
      <NavigationButtons
        :ConditionsMet="isEmpByFamilyMember === null ? true : false"
      ></NavigationButtons>
    </div>
  </div>
</template>
